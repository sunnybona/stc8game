...
//      delay(2000);
	...
	while (1==1) {
		...
		lastFrame=millis();
              ...
              if (RIGHT==0){if (player <128-platformWidth){player++;} if (player <128-platformWidth){player++;} if (player <128-platformWidth){player++;}}
              if (LEFT==0){if (player >0){player--;} if (player >0){player--;} if (player >0){player--;}}
...
void resetGame(){
  ...
  ssd1306_char_f6x8(22, 7, "Stc8 Arcade"); 
  waitStart();
 
//  beep(200,600);          beep(300,200);          beep(400,300);
  delay(2000);
  for (i =0; i<16;i++){ // reset blocks
   row1[i]=1; row2[i]=1; row3[i]=1;
  } 
  platformWidth = 16;
  ballx = 64;
...
void drawPlatform(){
 ...
 ssd1306_send_data_start();
 for (pw = 1; pw <platformWidth; pw++){ssd1306_send_byte(B00000011);}                
 ssd1306_send_data_stop();  
